...
public class JwtUtil {
	...
	public static final long JWT_TOKEN_VALIDITY = TimeUnit.DAYS.toMillis(30);
	private static final String SECRET = "qwertyuiop";
	private static final String ENCODED_JWT_SECRECT = Base64.getEncoder().encodeToString(SECRET.getBytes());
	...
	private Claims getAllClaimsFromToken(String token) {
		Claims claims = null;
		try {
		claims = Jwts.parser().setSigningKey(ENCODED_JWT_SECRECT).parseClaimsJws(token).getBody();
		}catch(Exception ex) {
